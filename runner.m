...
% Get values to plot
mean_r = mean( history(:, :, 1), 2 );
mean_s = mean( history(:, :, 2), 2 );
mean_t = mean( history(:, :, 3), 2 );
mean_p = mean( history(:, :, 4), 2 );

mean_payoff = mean( [mean_r, mean_s, mean_t, mean_p] .* repmat( payoff, n_iterations, 1 ), 2 );
...
% Plot r, s, t and p
subplot(5, 1, 2);
plot( mean_r ); ylabel('r1');
subplot(5, 1, 3);
plot( mean_s ); ylabel('s1'); 
subplot(5, 1, 4);
plot( mean_t ); ylabel('t1'); 
subplot(5, 1, 5);
plot( mean_p ); ylabel('p1'); 

%% Save to a nice .png file
print -dpng
